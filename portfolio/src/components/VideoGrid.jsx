export default function VideoGrid({ videos, fallbackVideos, isSanity }) {
  ...
  const sanityItems = isSanity ? videos : [];
  const items = [...sanityItems, ...fallbackVideos];
  ...
  return (
    <section className="w-full py-16 px-4 md:px-12 lg:px-24 border-b border-pink-200/50 bg-[#FFF0F5]/50">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
        {items.map((item, i) => {
          const videoUrl = item.videoUrl || item.src || item;
          const isNativePlay = item.playOnSite || (isSanity && item.videoUrl); // Sanity videos play on site by default
              ...
              <div className="group overflow-hidden rounded-xl glass cursor-pointer aspect-[9/16] relative bg-black/5 shadow-lg">
                {isNativePlay ? (
                  ...
                  >
                    ...
                    <div className="absolute bottom-4 left-4 right-4 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none text-center">
                      <p className="text-[10px] uppercase tracking-widest font-bold drop-shadow-md">Watch on Socials</p>
                    </div>
                  </a>
                )}
              </div>
